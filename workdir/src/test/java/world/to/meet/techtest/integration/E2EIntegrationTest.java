...
public class E2EIntegrationTest {
    ...
    public void testFindAllEndpointInvokesControllerMethod() throws Exception {
        // Perform GET request to /api/ships
        mockMvc.perform(MockMvcRequestBuilders.get("/api/ships"))
    ...
    @Sql(value = {"/create_spacial_ship_test_table.sql"}, executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
    @Sql(value = {"/delete_spacial_ship_test_table.sql"}, executionPhase = Sql.ExecutionPhase.AFTER_TEST_METHOD)
    public void testFindAllEndpointInvokesControllerMethod2() throws Exception {
        // Perform GET request to /api/ships
        mockMvc.perform(MockMvcRequestBuilders.get("/api/ship/pageable")
                        .param("page", "0")
                        .param("size", "10")
                ).andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.length()").value(2));
    ...
    public void testFindByNameEndpointInvokesServiceFindByName() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/api/ships/findByName").param("nombre", shipName))
                .andExpect(MockMvcResultMatchers.status().isOk())
    ...
    public void testCreateEndpointInvokesServiceCreate() throws Exception {
        // Perform POST request to /api/ships
        mockMvc.perform(MockMvcRequestBuilders.post("/api/ships")
    ...
    public void testUpdateEndpointInvokesServiceUpdate() throws Exception {
        Long shipId = 1L;
        mockMvc.perform(MockMvcRequestBuilders.put("/api/ships/{id}", shipId)
                        .contentType("application/json")
                        .content("{ \"name\": \"X-Wing\", \"model\": \"T-65B X-wing\", \"type\": \"Incom Corporation 2\" }"))
                .andExpect(MockMvcResultMatchers.status().isOk());

        // Perform GET request to /api/ships/{id}
        mockMvc.perform(MockMvcRequestBuilders.get("/api/ships/{id}", shipId))
                .andExpect(MockMvcResultMatchers.status().isOk())
    ...
    public void testDeleteEndpointInvokesServiceDelete() throws Exception {
        Long shipId = 1L;
        // Perform DELETE request to /api/ships/{id}
        mockMvc.perform(MockMvcRequestBuilders.delete("/api/ships/{id}", shipId))
